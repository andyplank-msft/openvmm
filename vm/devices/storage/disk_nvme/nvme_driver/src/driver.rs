...
impl<T: DeviceBacking> NvmeDriver<T> {
    ...
    pub async fn restore(
        ...
        let offset = match devid {
            ...
            "c05b:00:00.0" => 0,           
            _=> { tracing::error!(dev_id, "Device ID is unknown. RESTORE"); 0}
        };
            ...
            .await
        {
            ...
            Err(err) => {
                ...
                let (fallback_cpu, fallback) = self.io_issuers.per_cpu[..96 as usize]
                    ...
                    .unwrap();
                    tracing::error!(cpu, devid=self.device.id(), "No fallback queue. Major error");
                    // let (fallback_cpu, fallback) = test.unwrap();
                fallback.clone()
            }
        };
